...
def create_app():
    ...
    ] = "https://cdn.jsdelivr.net/npm/swagger-ui-dist@3.20.3/"
    ...
    def spendanalysis():
        ...
        form.select_account.choices = [("0", "---")] + [
            (i.account_id, i.account_no) for i in user.accounts
        ]
        if form.validate_on_submit():
            ...
            if from_date > to_date:
                ...
                txns = Acc_Transaction.query.filter(
                    Acc_Transaction.acc_id == cur_acc,
                    Acc_Transaction.txn_date.between(from_date, to_date),
                )
                if form.from_amt.data:
                    if not form.to_amt.data:
                        txns = txns.filter(
                            db.or_(
                                Acc_Transaction.withdrawal_amt >= form.from_amt.data,
                                Acc_Transaction.deposit_amt >= form.from_amt.data,
                            )
                        )
                    else:
                        if form.from_amt.data < form.to_amt.data:
                            txns = txns.filter(
                                db.or_(
                                    Acc_Transaction.withdrawal_amt.between(
                                        form.from_amt.data, form.to_amt.data
                                    ),
                                    Acc_Transaction.deposit_amt.between(
                                        form.from_amt.data, form.to_amt.data
                                    ),
                                )
                            )
                            ...
                            messages[
                                "longmsg"
                            ] = "To amount cannot be greater than From amount"
                if "msg_stat" not in messages:
                    if form.srch_remarks.data:
                        txns = txns.filter(
                            Acc_Transaction.txn_remarks.ilike(
                ...
                if "msg_stat" not in messages:
                    if txns.all():
                        display = True
                        top5cr = (
                            txns.filter(Acc_Transaction.withdrawal_amt == 0)
                            .order_by(Acc_Transaction.deposit_amt.desc())
                            .limit(5)
                            .all()
                        )
                        top5dr = (
                            txns.filter(Acc_Transaction.deposit_amt == 0)
                            .order_by(Acc_Transaction.withdrawal_amt.desc())
                            .limit(5)
                            .all()
                        )
                        first_txn = txns.order_by(Acc_Transaction.txn_date).first()
                        last_txn = txns.order_by(
                            Acc_Transaction.txn_date.desc()
                        ).first()
                        credit_summary = (
                            txns.with_entities(
                                db.func.sum(Acc_Transaction.deposit_amt).label("Sum"),
                                db.func.count(Acc_Transaction.txn_id).label("Count"),
                            )
                            .filter(Acc_Transaction.withdrawal_amt == 0)
                            .all()
                        )
                        debit_summary = (
                            txns.with_entities(
                                db.func.sum(Acc_Transaction.withdrawal_amt).label(
                                    "Sum"
                                ),
                                db.func.count(Acc_Transaction.txn_id).label("Count"),
                            )
                            .filter(Acc_Transaction.deposit_amt == 0)
                            .all()
                        )
                        ...
                        if first_txn:
                            form.opening_bal.data = (
                                first_txn.balance
                                + first_txn.withdrawal_amt
                                - first_txn.deposit_amt
                            )
                            res_view["opening_bal"] = (
                                first_txn.balance
                                + first_txn.withdrawal_amt
                                - first_txn.deposit_amt
                            )
                        ...
                        if form.closing_bal.data >= form.opening_bal.data:
                            form.balance.data = (
                                form.closing_bal.data - form.opening_bal.data
                            )
                            res_view["balance"] = (
                                form.closing_bal.data - form.opening_bal.data
                            )
                            ...
                            form.balance.data = (
                                form.opening_bal.data - form.closing_bal.data
                            )
                            res_view["balance"] = (
                                form.opening_bal.data - form.closing_bal.data
                            )
                        ...
                        messages[
                            "longmsg"
                        ] = "No Values found in database for above search criteria"
            ...
            if form.select_account.data == 0:
                ...
                try:
                    ...
                    selected_acc = Account.query.filter_by(
                        account_id=form.select_account.data
                    ).first()
                    ...
                    messages[
                        "longmsg"
                    ] = "{} records successfully inserted. {} Records failed".format(
    ...
    def addbank():
        ...
        bankform = BankList()
        bankform.bankname.choices = [("0", "---")] + [
            (bank.bank_id, bank.bank_name) for bank in BankDetails.query.all()
        ]

        if (
            bankform.validate_on_submit()
            and bankform.refresh.data
            and bankform.bankname.data != 0
        ):
            ...
            for account in delform.bank_det:
                ...
                if account.deactivate.data:
                    ...
                    delacc = Account.query.filter_by(
                        account_no=account.accountno.data
                    ).first()
                    delacc.active = False
                    try:
                        ...
                        if acc_exist:
                            if acc_joint:
                                ...
                                if x == 1:
                                    ...
                                    messages[
                                        "longmsg"
                                    ] = "Account {} already exists for current user. Skipping...".format(
                                        bank.accountno.data
                                    )
                                else:
                                ...
                                messages[
                                    "longmsg"
                                ] = "Account {} already assigned to another user!".format(
                                    bank.accountno.data
                                )
                        else:
                            ...
                            try:
                                ...
                                messages[
                                    "longmsg"
                                ] = "Accounts Added to user {}".format(cur_user["name"])
                    else:
                        messages["msg_stat"] = "alert-danger"
                        messages["shortmsg"] = "Error!"
